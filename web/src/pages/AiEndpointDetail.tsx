...
// A fully-populated default so the page renders for services that have no
// stored AI config yet — GET /services/{id}/ai-config returns {} for those.
// Configured services PUT a complete blob, so their GET is returned as-is.
const DEFAULT_AI_CONFIG: ServiceAIConfig = {
  cache: {
    enabled: false, applies_per: "per_endpoint", ttl_seconds: 600, max_entries: 1000, max_per_entry_kb: 64,
    semantic: {
      enabled: false, min_similarity: 0.85, embedding_mode: "local",
      embedding_url: "http://localhost:11434/v1/embeddings", embedding_model: "nomic-embed-text",
      fallback_policy: "treat_as_miss", promote_on_miss: true, max_index_entries: 10000,
    },
  },
...
export default function AiEndpointDetail() {
  ...
  const cfg = useQuery({
    ...
    queryFn: async () => {
      ...
      // Unconfigured services return {} (or null) — fall back to a complete
      // default so the routing/cache/etc. sections render without crashing.
      return c && Object.keys(c).length > 0 ? (c as ServiceAIConfig) : DEFAULT_AI_CONFIG;
    },
    retry: false,
    enabled: Boolean(id),
  });
