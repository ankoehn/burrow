...
//   no skip if absent â€” MCP page is not yet surfaced in the dashboard).
...
test("26-mcp: tool inventory â€” API returns â‰¥10 tools; UI surface optional", async ({ page, request }) => {
  // 1. API: backend must expose â‰¥10 MCP tools.
  ...
  expect(tools.length, `tool count (${tools.length}) â‰¥ 10`).toBeGreaterThanOrEqual(10);

  // 2. UI surface: optional â€” check /settings for an MCP card.
  ...
  if (!hasCard) {
    // No MCP UI surface in this build â€” API coverage above is the full test.
    return;
  }

  // MCP card found â€” click through and assert heading + tool listing.
  await mcpCard.click();
